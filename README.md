...
- **Struct types** — in progress, across several small milestones. Currently landed: top-level `struct Name { field* }` declarations with duplicate-struct-name/duplicate-field-name checking (Milestone S1), plus struct names now parsing as a variable/parameter/return type (Milestone S2) — though semantic analysis still rejects every such use with a temporary "struct types are not supported yet" diagnostic. **Not yet usable**: no struct-typed declaration actually works yet, there is no construction syntax, and there is no field access or assignment — see [LANGUAGE.md's Structs section](LANGUAGE.md#structs).

...
public final class Parser {
    ...
    private FunctionDeclaration parseFunction() {
        SourceLocation start = peek().location();
        TypeRef returnType = parseDeclaredType();
    ...
    private List<Parameter> parseParameters() {
        ...
        if (!check(TokenType.RIGHT_PAREN)) {
            do {
                ...
                TypeRef type = parseDeclaredType();
    ...
    // field ::= declaredType IDENTIFIER ";" — the exact same shape as a
    // function parameter, so it's represented as a Parameter (see
    // parseParameters()); no separate "Field" AST node exists. As of
    // Milestone S3, fields go through the same declaredType production as
    // every other type position (a struct field may itself be struct-typed
    // or array-of-struct-typed — see SemanticAnalyzer's cycle detection for
    // what that does and doesn't allow).
    ...
    // later, at array-creation time via "new elementType[size]"). The
    // primitive half of declaredType below.
    ...
    // declaredType ::= type | IDENTIFIER ( "[" "]" )? — the ONE type
    // production used for every type position: a field, a parameter, a
    // return type, or a variable. A bare IDENTIFIER here is unambiguously a
    // struct name, since the grammar already mandates a type at these
    // positions (no lookahead needed, unlike at statement-start — see
    // isStructTypedDeclarationStart()); an optional trailing "[]" marks
    // "array of that struct", exactly mirroring how a primitive type's own
    // array suffix works.
    private TypeRef parseDeclaredType() {
        if (check(TokenType.IDENTIFIER)) {
            ...
            // PrimitiveType.BOOL here is an arbitrary placeholder, not a real
            // element type — see TypeRef's own javadoc: elementType() has no
            // meaning once structName() is present.
            return new TypeRef(PrimitiveType.BOOL, isArray, Optional.of(name.lexeme()));
        }
        return parseType();
    }
    ...
    // call) all start the same way. Two IDENTIFIERs in a row is one shape
    // that means "declaration" (struct name, then variable name). As of
    // Milestone S3, "Point[] arr;" is also a declaration — it starts
    // IDENTIFIER LEFT_BRACKET, the same as "arr[i] = 5;" (an index
    // assignment), so a THIRD token is needed to tell them apart: an index
    // expression can never be empty ("arr[]" is always a parse error), so
    // IDENTIFIER LEFT_BRACKET RIGHT_BRACKET is unambiguously a type suffix,
    // not an index. Still fixed lookahead, no backtracking.
    private boolean isStructTypedDeclarationStart() {
        if (!check(TokenType.IDENTIFIER)) {
            return false;
        }
        if (checkAt(1, TokenType.IDENTIFIER)) {
            return true;
        }
        return checkAt(1, TokenType.LEFT_BRACKET) && checkAt(2, TokenType.RIGHT_BRACKET);
    }

    private boolean checkAt(int offset, TokenType type) {

...
- **`Expr`** (sealed, 11 concretes) — the original seven (`LiteralExpression`, `VariableExpression`, `GroupingExpression`, `UnaryExpression`, `BinaryExpression`, `AssignmentExpression`, `FunctionCallExpression`) plus four array-specific nodes added in v2 Milestone 1: `NewArrayExpression` (`new elementType[size]`), `ArrayAccessExpression` (`array[index]` as a read), `ArrayLengthExpression` (`array.len()`), and `IndexAssignmentExpression` (`array[index] = value`, kept as its own node rather than generalizing `AssignmentExpression`'s `String` target to an arbitrary lvalue — leaves the original, well-tested plain-variable assignment path untouched). Unchanged by v3 Milestone S1 — struct declarations don't add a new expression or statement kind.
...
- **`StructDeclaration`** (v3 Milestone S1) — a top-level `struct Name { field* }` declaration. Each field is represented as a plain `Parameter` (type + name) rather than a new "Field" record, since the shapes are identical — mirrors how `FunctionDeclaration.parameters()` already works. Declarations-only for now: no AST node yet exists for struct construction or field access (those are later milestones).
...
- **`Opcode`** — the closed instruction set: 38 opcodes across constants, locals, arithmetic, comparison, control flow, functions, arrays, I/O, and stack operations. Deliberately small compared to JVM bytecode — every type-specific decision (which numeric type, which operator variant) was already resolved by semantic analysis, so the instruction set itself doesn't need per-type opcode variants. The four array opcodes (`NEW_ARRAY`/`ARRAY_GET`/`ARRAY_SET`/`ARRAY_LENGTH`, added in v2 Milestone 1) operate on a plain `Object[]` reference — GopiLang arrays have no dedicated runtime wrapper type, consistent with every scalar already being a generically-boxed `Object` in the VM.

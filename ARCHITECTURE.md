...
- **`Parameter`, `VariableDeclaration`, `FunctionDeclaration`** — as of v3 Milestone S2, each gained an `Optional<String>` struct-type-name field (`structTypeName`/`structReturnTypeName`) alongside its existing `TypeRef` field, populated when the parser read a struct name in type position instead of a primitive keyword. The `TypeRef` field itself is untouched in shape and always still populated — with `Parser.STRUCT_TYPE_PLACEHOLDER` for a struct-typed declaration — specifically so `CodeGenerator`'s existing `.type()`/`.returnType()` call sites need no changes. `SemanticAnalyzer` rejects every `structTypeName`/`structReturnTypeName` occurrence immediately (see below), so the placeholder is never read as a real type for a program that reaches code generation.
...
- **`TypeRules`** — pure static type-compatibility rules (assignability, binary/unary operator result types, printability), with zero dependency on `Scope`/`SemanticModel`/the AST — kept separate specifically so it's independently testable. `resultOfUnary`/`resultOfBinary` stay `PrimitiveType`-only (arrays never reach them — `typeOf` rejects an array operand for every operator before calling in); `isAssignable`/`isReturnCompatible`/`isArgumentCompatible`/`isPrintable` are `TypeRef`-based, since those checks do need to know about arrays (array assignment requires an exact element-type match, with no `int`-to-`float`-style widening). Untouched by v3 Milestones S1 and S2 — `TypeRef` itself still doesn't change; a struct name in type position never reaches `TypeRules` at all, since `SemanticAnalyzer`'s temporary stub rejects it immediately instead.
- **`FunctionSymbol`**, **`VariableSymbol`** — resolved-declaration records (name, type, declaration site) that `SemanticModel`'s maps point at. Their `type`/`returnType`/`parameterTypes` fields are `types.TypeRef` (element type + "is this an array"), not bare `PrimitiveType` — the minimum type-system extension arrays require, added in v2 Milestone 1 and threaded through everywhere a declared or computed type is stored (`Parameter`, `VariableDeclaration`, `FunctionDeclaration`, `SemanticModel.expressionTypes()`).
...
- **`AstPrinter`** — renders a `Program` as a Unicode tree. Deliberately external to the `ast` package, dispatching with a plain `switch` over the sealed `Stmt`/`Expr` hierarchies rather than adding an `accept()` method to the AST nodes. As of v3 Milestone S2, its `Parameter`/`VariableDeclaration`/`FunctionDeclaration` labels print the struct name (via the small `typeLabel()` helper) whenever `structTypeName`/`structReturnTypeName` is present, since the underlying `TypeRef` is only a parser placeholder in that case and would otherwise print misleadingly.

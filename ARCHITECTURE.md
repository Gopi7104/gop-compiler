...
- **`Lexer`** — hand-written character-by-character scanner. Recognizes keywords, identifiers, literals (`int`, `float`, `string`, `bool`), operators, and punctuation. Skips whitespace and `//` line comments. Recovers from a bad character or malformed literal via panic-mode: it catches its own `LexerException`, converts it to a `Diagnostic`, and resumes scanning from the next character — so one bad token doesn't abort the whole file.
